...
namespace HMTLPLUS {
    void header(
        std::ostream& html_file, const std::string& title,
...
<body>
    ...
    void
    page_body(std::ostream& html_file, const std::vector<string>& paragraphs) {
    ...
    void index_body(
        ...
        <ul>)";
            for (const auto& file : files) {
                html_file
                    << R"(<li><a href=")" << file
                    << R"(.html"><i class="fa-regular fa-file-lines"></i> )"
                    << file << R"(</a></li>)";
            }
        }
        html_file << R"(
