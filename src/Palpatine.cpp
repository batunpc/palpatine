    ...
    if (fs::is_directory(input)) {
        ...
        for (const auto& entry : fs::directory_iterator(input)) {
            ...
            if (entry.is_directory()) {
                ...
                process_path(
                    input_path.string(), output_path.string(), "index.html");
        ...
        utils_sdds::print_error(
            "Error: " + input + " is not a valid file type");
        std::terminate();
    }
}

void Palpatine::generate_index_file(
    string output, const string& title, const vector<string>& directories,
